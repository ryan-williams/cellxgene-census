...
def tables_to_np(tables: Iterator[Tuple[Table, any]], shape: Tuple[int, int]) -> typing.Generator[Tuple[np.ndarray, any], None, None]:
...
class _ObsAndXSOMAIterator(Iterator[_SOMAChunk]):
    ...
    def __next__(self) -> _SOMAChunk:
        ...
        with profile("X_batch", append=True):
            X_batch: ChunkX = next(batch_iter)[0]
        checkpoint("X_batch")
